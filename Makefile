...
PACKFINALCODERSTATE=final-coder-state.bin
...
$(PROJECTTARGET): $(PROJECTSOURCE) $(COMMONINC) $(BASICINC) $(INTROTARGET) $(PACKEDINTRO) $(PACKFINALCODERSTATE)
	kickass -showmem $(PROJECTSOURCE)

$(PACKEDINTRO) $(PACKFINALCODERSTATE) $(PACKREPORT): packer $(INTROTARGET)
	cd $(PACKERDIR) && cargo run $(PACKERFLAGS) -- $(CURDIR)/$(INTROTARGET) $(CURDIR)/$(PACKEDINTRO) $(CURDIR)/$(PACKFINALCODERSTATE) $(CURDIR)/$(PACKREPORT) $(PACKERPROFILE)
